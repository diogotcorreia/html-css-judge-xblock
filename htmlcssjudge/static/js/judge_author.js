function HtmlCssJudgeXBlock(runtime, element, context) {
  ...
  function save_settings(cb) {
    ...
    if (context.uses_grader) data["grader_code"] = editor_grader.getValue();
    if (!context.no_submission)
      data["model_answer"] = editor_model_answer.getValue();

    const handlerUrl = runtime
      .handlerUrl(element, "save_settings")
      .replace("/preview", "");
  ...
  $(element)
    .find("#save-button_" + id)
    .bind("click", save_settings);

  // submit
  $(element)
    .find("#test_model_answer_" + id)
    .bind("click", function () {
